...
use lsn_cryptanalysis::{bkw_results_to_json, run_bkw_bucket_trials};
...
fn main() {
    let args = parse_args(env::args().skip(1).collect()).unwrap_or_else(|err| {
        eprintln!("{err}");
        eprintln!(
            "usage: lsn_bkw_sweep --n-start 4 --n-end 8 --ratios 1.0 --p-values 0.0,0.25 --trials 20 --bucket-bits 6 --pair-limit 256 --seed 3235823838 --output experiments/out.json"
        );
        std::process::exit(2);
    });
    ...
    for n in args.n_start..=args.n_end {
        ...
        for &ratio in &args.ratios {
            ...
            for &noise_rate in &args.noise_rates {
                eprintln!(
                    "running bkw n={n}, m={sample_count}, ratio={ratio:.3}, p={noise_rate:.4}, trials={}, bucket_bits={}, pair_limit={}",
                    args.trials, args.bucket_bits, args.pair_limit_per_bucket
                );
                results.push(run_bkw_bucket_trials(
                    n,
                    sample_count,
                    noise_rate,
                    args.trials,
                    args.bucket_bits,
                    args.pair_limit_per_bucket,
    ...
    let json = bkw_results_to_json("codex-p2-bkw-bucket-pair-screen", &results);
...
fn parse_args(raw: Vec<String>) -> Result<Args, String> {
    ...
    let mut trials = 20;
    let mut bucket_bits = 6;
    let mut pair_limit_per_bucket = 256;
    ...
    while i < raw.len() {
        ...
        match key.as_str() {
            ...
            "--bucket-bits" => bucket_bits = parse_value(key, value)?,
            "--pair-limit" => pair_limit_per_bucket = parse_value(key, value)?,
    ...
    if pair_limit_per_bucket == 0 {
        return Err("require positive pair limit".to_string());
    }
