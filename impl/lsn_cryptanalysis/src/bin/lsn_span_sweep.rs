...
use lsn_cryptanalysis::{run_span_trials, span_results_to_json};
...
fn main() {
    let args = parse_args(env::args().skip(1).collect()).unwrap_or_else(|err| {
        eprintln!("{err}");
        eprintln!(
            "usage: lsn_span_sweep --n-start 3 --n-end 5 --ratios 4.0 --p-values 0.0,0.25 --trials 10 --seed 3235823838 --output experiments/out.json"
        );
        std::process::exit(2);
    });
    ...
    for n in args.n_start..=args.n_end {
        ...
        for &ratio in &args.ratios {
            ...
            for &noise_rate in &args.noise_rates {
                eprintln!(
                    "running span n={n}, m={sample_count}, ratio={ratio:.3}, p={noise_rate:.4}, trials={}",
                    args.trials
                );
                results.push(run_span_trials(
                    n,
                    sample_count,
                    noise_rate,
                    args.trials,
    ...
    let json = span_results_to_json("codex-p2-span-positives-negative-control", &results);
