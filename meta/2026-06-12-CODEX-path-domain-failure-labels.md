# CODEX path-domain failure labels
...
path-buffer schedule domain checker.
...
`fixed_scl_path_domain_failure_label` exposes the label lookup for audit and
test code. Unknown codes resolve to `unknown`.

## Boundary

This is metadata for the source-level SCL audit rail. It does not change
`decode_scl`, `decode_scl_fast`, path scoring, or active decoding behavior.
...
- `fixed_scl_path_domain_failure_labels_cover_public_codes` failed because the
  label struct, table, and lookup function were absent.

GREEN:

- the label table covers every public path-domain failure code;
- unknown codes map to `unknown`;
- the SCL audit JSON now includes `public_path_domain_failure_codes`.

## Verification

Focused verification:

- `cargo test --manifest-path impl/polar_validation/Cargo.toml fixed_scl_path_domain_failure_labels_cover_public_codes`
...
The next bounded step is to add an analogous child-write failure-code label
table, or to continue removing tiny panic-only source helper surfaces with
explicit public domain checks.
