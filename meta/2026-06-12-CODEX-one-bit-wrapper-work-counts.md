# CODEX One-Bit Wrapper Work Counts
...
- Added `FixedSclOneBitExpansionRun.work_counts` so the non-panicking one-bit
  expansion wrapper exposes its public fixed work shape directly.
- Valid public one-bit domains report one-round public work counts.
- Invalid public path domains return empty child buffers, sentinel `top` entries,
  and zero-round public work counts.
- Extended the SCL work-shape audit wrapper map for
  `try_expand_then_compact_one_bit` with
  `FixedSclOneBitExpansionRun.work_counts`.

## RED/GREEN

- RED: `cargo test --manifest-path impl/polar_validation/Cargo.toml fixed_scl_path_buffer_try_expand_then_compact_one_bit`
  failed because `FixedSclOneBitExpansionRun` had no `work_counts` field.
- GREEN: the same focused test passed after adding the field and zero/full
  work-count behavior.
- GREEN: `scl_work_shape_audit_records_non_constant_time_surfaces` passed after
  the audit map update.

## Verification

- `CARGO_TARGET_DIR=/tmp/lsn-pq-polar-target cargo test --manifest-path impl/polar_validation/Cargo.toml fixed_scl_path_buffer_try_expand_then_compact_one_bit`
