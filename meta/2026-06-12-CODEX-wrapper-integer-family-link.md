# CODEX Wrapper Integer Failure-Family Link

Date: 2026-06-12

## Scope

This OFA-sized increment tightens the fixed SCL audit JSON after the integer
schedule domain gained explicit failure codes.

`non_panicking_wrapper_failure_code_map` now points the integer schedule side of
`try_expand_then_compact_integer_round_schedule` at
`integer_schedule_domain_failure_codes`, rather than the older
`integer_schedule_domain_check` label.
...
- Updated `scl_work_shape_audit_records_non_constant_time_surfaces` to require
  `"integer_status_family": "integer_schedule_domain_failure_codes"`.
- Focused test failed because the audit JSON still emitted
  `integer_schedule_domain_check`.

GREEN:

- Updated `scl_work_shape_audit_json()` to reference the explicit integer
  failure-code family.
