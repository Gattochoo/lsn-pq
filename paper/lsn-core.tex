...
What this does not close: the multi-secret-pair level. Reproducible enumeration code and exact-fraction data for all claims in this section are included with the code repository.
...
\item \textbf{Moment results close the pairwise level; the multi-pair level is open.} The closed forms of \Cref{sec:moments} are exact for every order $j$, \Cref{prop:vmax} controls the variance multiplier at the maximal block, \Cref{prop:tdist} recovers the full exact law of the quadrant count (with the proven TV rate $2^{-(n+1)}$), and \Cref{thm:joint-gf} closes the entire joint four-category composition. What remains open is the multi-secret-pair joint level.
