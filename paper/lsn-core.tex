...
\paragraph{Applications.} LSN supports cryptographic constructions --- a key-encapsulation mechanism via concatenated polar-code reconciliation and a succinct argument system exploiting the Siegel-chart representation of Lagrangians --- which are developed in a separate companion report available with the code repository. The present paper is concerned only with the hardness landscape of the problem itself.
...
$V_k = \sum_{j=0}^{k}\binom{k}{j}\sigma^{2j} m_j$, which controls the pairwise SQ correlation of $k$-row bundle examples drawn from the isotropic ensemble. For every fixed $k \le 3$\,--\,moment order,
...
\paragraph{\textbf{Data and code availability.}} All supporting material --- Python enumeration and verification scripts, Rust implementations (cryptanalysis screens, polar-code validation, and a reference scaffold), SageMath symbolic checks, exact-fraction data and test vectors, and the \LaTeX{} source --- is available at \url{https://github.com/Gattochoo/lsn-pq} and archived at \href{https://doi.org/10.5281/zenodo.20646796}{\texttt{doi:10.5281/zenodo.20646796}}. In particular, the closed forms of \Cref{sec:moments} are reproducible from the enumeration and symbolic-verification scripts.
