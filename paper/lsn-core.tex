...
Two remarks. First, the moment formula is exact for every fixed $j$; the only regime it does not directly control is $j=\Theta(n)$ (growing with the ambient dimension), which would govern bundles of growing size. Second, exact enumeration further shows that the bundle correlation is \emph{sub-multiplicative} in $k$ --- the isotropic coupling slightly \emph{reduces} correlation relative to independent rows --- so the conditioning helps rather than harms the lower-bound side at every measured size; we do not yet have a closed form for this suppression. Reproducible enumeration code and exact-fraction data for all claims in this section are included with the code repository.
...
\item \textbf{Moment results cover fixed order.} The closed forms of \Cref{sec:moments} are exact for every fixed order $j$; only the regime $j = \Theta(n)$ (growing with the ambient dimension), which governs bundles of growing size, is not directly controlled by them.
