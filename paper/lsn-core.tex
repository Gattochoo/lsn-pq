...
Two remarks. First, \Cref{prop:vmax} does \emph{not} close the full $j=\Theta(n)$ regime: it controls one functional (the variance multiplier) at growing block size, while the individual moments near $j = 2n$ deviate by $\Theta(1)$ relative factors ($m_j = 0$ for $j \ge 2n-1$, whereas $(1/4)^j > 0$) --- their weight in $V_{2n}$ is simply exponentially small. The full distribution of growing bundles (higher cumulants) remains open. Second, the suppression is consistent in sign at every scale: exact enumeration shows the bundle correlation is \emph{sub-multiplicative} in $k$ --- the isotropic coupling slightly \emph{reduces} correlation relative to independent rows --- and \Cref{prop:vmax} now gives this suppression a closed form at the maximal block, $-2(25/64)^n(1+o(1))$ relative at $p=1/4$: the conditioning helps rather than harms the lower-bound side. Reproducible enumeration code and exact-fraction data for all claims in this section are included with the code repository.
...
\item \textbf{Moment results cover fixed order plus one growing-block functional.} The closed forms of \Cref{sec:moments} are exact for every fixed order $j$, and \Cref{prop:vmax} controls the variance multiplier exactly even at the maximal block $k=2n$ (exponentially small deviation). What remains open in the $j=\Theta(n)$ regime is the full distribution of growing bundles: individual moments near $j=2n$ deviate by $\Theta(1)$ relative factors, and higher cumulants are not controlled by the variance summary alone.
