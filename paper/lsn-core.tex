...
Motivation: $k=2$ isotropic pencils force any threshold above $4\rho_{\mathrm{avg}}$ (their expected $2^{\dim(L\cap L')}$ ratio tends to $4$ from below), while $k \geq 3$ pencils fall below the size threshold $|\Lagr|/2^{2n}$ for $n \geq 4$, and mixtures of pencils with random fill dilute quadratically. A proof would require classifying near-extremal subsets. Exact small-case verification (evidence, not proof) supports the conjecture: at $n=2$, exhaustive enumeration of all $2^{15}$ subsets gives maximum ratio $5/2 < 5$ (attained only by singletons; the size-$3$ maximisers are exactly the $k=1$ pencils); at $n=3$, the exact maxima over all subsets of sizes $3$ and $4$ are $3$ and $81/32$ (attained by $k=2$ pencils), and pre-registered random/greedy search over larger sizes found nothing above ratio $3$.
...
\]
with $m_j$ from \Cref{thm:mj-general} --- an exact rational expression for every $n$ (e.g.\ $(11/45,\,4/9,\,14/45,\,0,\,0)$ at $n=2$). Consequently \emph{every} statistic of the pair-level quadrant count --- all moments, all cumulants, $V_k$ for every $k$ --- is exactly computable. The exact total-variation distance to the unconstrained law $\mathrm{Bin}(2n,1/4)$ is $707/5760 \approx 0.1227$ at $n=2$ and decays empirically like $2^{-(n+1)}$ ($2^n \cdot \mathrm{TV} = 0.491, \dots, 0.498$ for $n \le 10$; the rate is evidence, not a proven limit).
\end{proposition}

\begin{proof}
$B_j = \E[\binom{t}{j}]$, so the formula is the standard binomial (inclusion--exclusion) inversion; it is exact because \Cref{thm:mj-general} is. The resulting laws were checked against direct enumeration of the isotropic-pair ensemble at $n = 2, 3, 4$ (identical rationals), and structurally for $n \le 10$: they sum to one, are non-negative, vanish for $\ell \ge 2n-1$, and re-contract to every $B_j$.
\end{proof}

What this does not close: the joint four-category row composition beyond $t$ (the counts $t_{10}, t_{01}, t_{00}$ jointly with $t$), and the multi-secret-pair level. Reproducible enumeration code and exact-fraction data for all claims in this section are included with the code repository.
...
\item \textbf{Moment results close the pair-level quadrant count; the joint composition is open.} The closed forms of \Cref{sec:moments} are exact for every order $j$, \Cref{prop:vmax} controls the variance multiplier at the maximal block, and \Cref{prop:tdist} recovers the full exact law of the pair-level quadrant count $t$ (hence every statistic that factors through $t$). What remains open at the pairwise level is the joint four-category row composition beyond $t$, and beyond that the multi-secret-pair joint level; the conjectured TV rate $2^{-(n+1)}$ to the unconstrained law is evidence, not proof.
