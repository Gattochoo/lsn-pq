...
What this does not close: the multi-secret level beyond triples, and any SQ statement built on these joint laws. Reproducible enumeration code and exact-fraction data for all claims in this section are included with the code repository.
...
\item \textbf{Moment results close the pairwise level and the first triple object; the general multi-secret level is open.} The closed forms of \Cref{sec:moments} are exact for every order $j$, \Cref{prop:vmax} controls the variance multiplier at the maximal block, \Cref{prop:tdist} recovers the full exact law of the quadrant count (with the proven TV rate $2^{-(n+1)}$), \Cref{thm:joint-gf} closes the joint four-category composition, and \Cref{thm:triple-gf} closes the triple composition. What remains open is the multi-secret level beyond triples and the SQ statements built on these joint laws.
