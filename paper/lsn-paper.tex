...
\;\le\;
\frac{\log_2 N(n)+m+\delta nm+O(1)}{0.094n}
...
\]
The entropy of $A$ is at most the log-support of the isotropic-frame distribution, $H(A)\le\log_2 N(n)$ (Lane~C).  Combining with the Fannes--Csisz\'{a}r bound $H(C)\ge nm-\delta nm-1$ and the chain rule,
\[
nm-\delta nm-1
\;\le\;\log_2 N(n)+m+nm-\E[k\mid A]\cdot0.094n+O(1),
\]
which rearranges to $\E[k\mid A]\cdot0.094n\le\log_2 N(n)+m+\delta nm+O(1)$.  Taking expectations over $A$ and $R$ and using $N(n)=2^{(3/2)n^{2}+n/2+O(1)}$ gives the claimed bound.
\end{proof}
...
\label{lem:m2}
Let $p'\in(0,1/2)$ with $1-2p'\ge 1/\operatorname{poly}(n)$.  Consider any reduction outputting $m$ samples $(c_i,a_i)$ with $a_i=\langle c_i,x\rangle+e'_i$ where $e'_i$ has bias $\le 2^{-0.19n}$ on all but $\ell=o(m)$ rows.  Then
\[
...
\begin{proof}
The reduction output is $(C,y)$ where $y_i=\langle c_i,x\rangle+e'_i$.  In the target $\mathrm{LPN}_{p'}$ distribution each row has independent noise with bias $1-2p'$.  In our output, at least $m-\ell$ rows have bias $\le 2^{-0.19n}=o(1)$.

Consider the max-agreement distinguisher: for each candidate secret $x'\in\F_2^{n}$, compute $S(x')=\sum_{i=1}^{m} \mathbf{1}_{y_i=\langle c_i,x'\rangle}$.  Under the true secret $x$, the expected agreement is
\[
\E[S(x)] \;\ge\; (1-p')(m-\ell) + \tfrac12\ell
\;=\; \frac{m}{2} + \Bigl(\tfrac12-p'\Bigr)(m-\ell).
\]
For any wrong secret $x'\neq x$, the labels $y_i$ are independent of $\langle c_i,x'\rangle$ on the low-bias rows (because the parity $\langle c_i,x+x'\rangle$ is unbiased when $c_i$ is uniform and $x\neq x'$), so $\E[S(x')]\le m/2$.

By Hoeffding's inequality and a union bound over the $2^{n}$ candidates,
\[
\Pr\bigl[S(x')\ge \E[S(x)] - t\bigr]
\;\le\; 2^{n}\exp\!\Bigl(-\frac{2t^{2}}{m}\Bigr).
\]
Choosing $t=\frac14(1-2p')(m-\ell)$ gives failure probability at most $2^{n}\exp(-\Omega((1-2p')^{2}m))$, which is $o(1)$ when $m\ge 4n/(1-2p')^{2}\cdot\operatorname{polylog}(n)$.  Hence the distinguisher separates the two distributions with advantage $1-o(1)$.
\end{proof}
