...
\begin{proof}
For real-linear queries, direct computation gives $\E_a[\langle w,a\rangle]=0$ and $\E_{D_L}[b]=p+2^{-n}(1-2p)$, which is independent of $L$. For an F$_2$-linear query $q(a,b)=\langle w,a\rangle\oplus c\cdot b$: if $c=0$ then $\E_{D_L}[q]=1/2$ for every $L$; if $c=1$ and $w=0$, then $q=b$ and the advantage over $D_0$ is exactly $(1-2p)2^{-n}$. If $c=1$ and $w\neq 0$, write $q=b\oplus\langle w,a\rangle$. Under $D_L$ we have
\[
...
The work of Lu \etal\ \cite{LPQR26} proves, on our reading, a strong barrier against \emph{linear} reductions sympLPN $\to$ LPN in the low-noise regime via the entropy-deficiency argument of Lu \etal\: isotropy constraints cost $C(n,2)$ bits of entropy, and any linear reduction destroys the error before uniformity is reached. Our \Cref{thm:linear-sq} gives a complementary, direct barrier in the SQ model.
...
\paragraph{The scrambling barrier at constant noise.} Lu \etal\ \cite{LPQR26} proved, as our reconstruction of their argument suggests, that \emph{linear} reductions sympLPN $\to$ LPN are impossible in the low-noise regime because isotropy constraints cost $C(n,2)$ bits of entropy and linear maps cannot randomize the public matrix before the noise is exhausted. At our constant noise $p=1/4$ this \emph{entropy argument disappears}: the total noise entropy $Nr \cdot H(p) \approx 1.8\times 10^4$ bits (for $n=65$) far exceeds the $C(n,2) = 2080$ bits of isotropy constraint. What survives is an \emph{algebraic} obstruction: $S_A=0$ is a deterministic quadratic relation among the public vectors. A linear scrambler preserves isotropy (detectable); a non-linear scrambler destroys the linear structure required by LPN; a random scrambler destroys the secret entirely. Direct inspection of the $n=3$ case shows that simple scramblers fall into one of three classes: a linear scrambler preserves isotropy (so $S_A=0$ remains detectable), a non-linear scrambler destroys the linear structure required by LPN, and a random scrambler destroys the secret entirely. None of these simple classes simultaneously destroys $S_A=0$ and creates a fresh LPN secret-bit structure. The residual question---whether a sufficiently complex non-linear scrambler exists---is the same as Open Problem~2.
