...
Any reduction from $\mathrm{LSN}(n)$ to $\mathrm{LPN}(k)$ that preserves the secret (i.e., an adversary solving the LPN instance can recover the LSN secret with non-negligible probability) must satisfy $k = \Omega(n^2)$.
\end{proposition}

\begin{proof}
The LSN secret space is $\Lagr(2n,\F_2)$ with $|\Lagr| = \prod_{i=1}^n (2^i+1)$. Hence $H(L) = \log_2|\Lagr| = n(n+1)/2 + \Theta(1)$, where the additive constant is $\log_2\prod_{i=1}^{\infty}(1+2^{-i}) \approx 1.25$. If a reduction outputs an $\mathrm{LPN}(k)$ instance whose secret determines $L$, Shannon's source coding theorem (or Fano's inequality) requires $k \geq H(L) - o(1) = \Omega(n^2)$.
\end{proof}

\paragraph{Concrete-security win-win.} For the parameter sets in \Cref{tab:parameters}, Proposition~\ref{prop:entropy-floor} forces $k \geq 862$ (80-bit), $k \geq 1432$ (100-bit), or $k \geq 2146$ (128-bit). The best known attack on $\mathrm{LPN}(k)$ (BKW \cite{BKW03}) requires time $2^{\Omega(k/\log k)}$. At these dimensions this yields $2^{88.4}$, $2^{136.6}$, and $2^{193.9}$ respectively---all exceeding the target security levels. We stress that this is a concrete-security observation, not an asymptotic impossibility: BKW on $\mathrm{LPN}(\Theta(n^2))$ runs in $2^{\Theta(n^2/\log n)}$, which is asymptotically smaller than brute-force on LSN ($2^{\Theta(n^2)}$). For the parameter range of interest, however, the entropy floor lands BKW well above the security budget.

\paragraph{Adaptivity does not increase degree.} A natural question is whether \emph{adaptive} degree-$D$ feature-map reductions circumvent the static degree barrier. They do not: in each round the reduction applies a feature map $\phi_j(a,b)$ to the raw samples $(a,b)$. The degree of $\phi_j$ is at most $D$, regardless of how $\phi_j$ was chosen. Composing $r$ such maps does not increase degree because the outputs of previous rounds are expectations (real numbers), not new samples; they are used to \emph{select} the next query, not to raise its algebraic degree. Consequently the effective degree of the entire transcript is bounded by $D$. When $D < n$, the Reed--Muller obstruction of \Cref{sec:polynomial} still applies: correlation with $\mathbf{1}_L$ is at most $2^{-n}$, requiring exponentially many samples. Thus adaptive degree-$D$ feature-map reductions are blocked for every $D < n$, independent of the number of rounds.

The remaining open question is whether a \emph{genuinely non-linear} reduction exists. We do not claim impossibility. Instead we observe a win-win structure: if such a reduction is found, it would likely improve random self-reductions for LPN itself, making it valuable even if it collapses LSN to the 6.5th family.
...
\item \textbf{$x$-free quadratic constraint.} The public matrix $A$ in LSN satisfies $S_A=0$ (the $\Omega$-Gram of columns is identically zero). This is a deterministic, public, quadratic constraint on the \emph{public parameters} alone. LPN has no analogous public structure linking its samples.
...
\Cref{conj:source} is falsifiable: a single explicit reduction sympLPN $\to$ LPN that preserves any of the three invariants would refute the conjecture in its current form. It is not decidable by reduction analysis alone; it rests on the absence of such a reduction and the structural barriers documented above.

\paragraph{The scrambling barrier at constant noise.} Lu \etal\ \cite{LPQR26} proved that \emph{linear} reductions sympLPN $\to$ LPN are impossible in the low-noise regime because isotropy constraints cost $C(n,2)$ bits of entropy and linear maps cannot randomize the public matrix before the noise is exhausted. At our constant noise $p=1/4$ this \emph{entropy argument disappears}: the total noise entropy $Nr \cdot H(p) \approx 1.8\times 10^4$ bits (for $n=65$) far exceeds the $C(n,2) = 2080$ bits of isotropy constraint. What survives is an \emph{algebraic} obstruction: $S_A=0$ is a deterministic quadratic relation among the public vectors. A linear scrambler preserves isotropy (detectable); a non-linear scrambler destroys the linear structure required by LPN; a random scrambler destroys the secret entirely. A computational probe at $n=3$ confirms that no simple scrambler (linear, quadratic, or random) simultaneously destroys $S_A=0$ and creates a fresh LPN secret-bit structure. The residual question---whether a sufficiently complex non-linear scrambler exists---is the same as Open Problem~2.
...
\item \textbf{Loose multi-user reduction.} \Cref{subsubsec:kem-multiuser} uses a hybrid argument losing a factor $N$. A tight reduction would require random self-reducibility. We observe that LSN does \emph{not} admit the standard LPN-style random self-reduction: because $\mathbf{1}_L$ is non-linear, noise cannot be refreshed by combining public samples without knowing $L$. In practice the margin is comfortable ($q\gtrsim 2^{114}$), but the bound is not tight.
