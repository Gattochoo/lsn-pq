...
A $\delta$-enrichment would scale the per-query advantage by $(1+\delta)$; for selection rules implementable by linear SQ this exceeds the per-query ceiling of \Cref{thm:linear-sq}, while general (non-linear, multi-sample) selection rules remain open.  Empirical probes at $n=5$ (full graph-Lagrangian prior, $32{,}768$ secrets) and $n=6$ (random subset, $100{,}000$ secrets) confirm the predicted scaling $\delta \approx c\,m\,2^{-n}$ with $c\approx 0.01\text{--}0.06$ (\cref{subsec:limitations}, Experiment~92).  At $n=65$ and $m=22{,}528$ this gives $\delta \le 2^{-50}$, far below any cryptographically relevant threshold.
...
Linear, $\rho \le (3/2-\epsilon)n$, $m=\omega(n)$ & CLOSED & Reachability bound; any-$B$, any $m=\operatorname{poly}(n)$ (\Cref{thm:any-b-uniformity}) \\
...
The preceding affine-coset bound controls the per-row label signal when the reduction is forced into a high-rank affine subspace.  A complementary barrier applies to \emph{any} $B$ model (adaptive or fixed, public or secret) and closes the linear-reduction landscape at constant noise $p=1/4$ for all $m=\operatorname{poly}(n)$.  It rests on a pure counting argument that is independent of $A$.
...
Let $A\in\F_2^{2n\times n}$ be any isotropic basis and let $B\in\F_2^{m\times 2n}$ be any matrix (possibly depending on~$A$).  Let $w=\lfloor 0.19n\rfloor$ and suppose that $\operatorname{SD}(BA,\,\operatorname{Uniform}_{\F_2^{n}})\le\delta$.  Then for every row $b_i$ of~$B$,
\[
\Pr\bigl[\,|b_i|\le w\,\bigr] \;\le\; \frac{|R_w(A)|}{2^{n}} + \delta \;\le\; 2^{-0.06n}+\delta.
\]
In particular, if $m\le\operatorname{poly}(n)$ and $\delta\le 1/(4mn)$, then with probability $1-o(1)$ over the randomness of the reduction \emph{every} row of $B$ has weight $>w$.
\end{theorem}
...
\Cref{thm:any-b-uniformity} applies to adaptive $B$, secret $B$, and any distribution over $B$ conditioned only on the marginal of $BA$ being uniform.  It is the final piece of the linear-reduction closure: once $BA$ is forced to be uniform (otherwise the output is distinguishable from standard LPN), every row of $B$ must have weight $>0.19n$.
...
Together, \Cref{lem:reachability,thm:any-b-uniformity,cor:noise-amp,cor:recovery-barrier} close the linear-reduction landscape: any linear reduction that makes $BA$ uniform is forced into high-weight rows, which amplifies the noise to $p'\ge \tfrac12-2^{-\Theta(n)}$, and recovery then requires $m=2^{\Theta(n)}$.  At $m=\operatorname{poly}(n)$ the reduction fails unconditionally, for every $B$ model, every isotropic $A$, and every noise rate $p=\Theta(1)$.
