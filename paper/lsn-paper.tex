...
\item $\rho \le (3/2-\epsilon)n$ (mid/low rank): \textbf{OPEN} --- the rank test dies ($2c \ge (1+\epsilon)n$). At $m=\Theta(n)$, LPQR26 rule out \emph{all} fixed linear reductions (Appendix~D, Theorem~D.1); at $m=\omega(n)$ their bound is insufficient. The entropy argument of \Cref{prop:entropy-floor} gives a complementary barrier. A quantitative bridge theorem that covers \emph{all} $B$-distributions simultaneously would close this strip entirely.
\end{itemize}
...
\begin{proof}
Write $b = b_0 + Nz$ with $z \sim \operatorname{Uniform}(\F_2^n)$ and $N$ a basis of $\operatorname{nullspace}(A^{\!\top})$. Then $\E_b[(-1)^{b^{\!\top} e}] = (-1)^{b_0^{\!\top} e} \cdot \mathbf{1}_{e \in \operatorname{colspace}(A)}$, so $|\E_{b,e}[(-1)^{b^{\!\top} e}]| = \sum_{x \in \operatorname{colspace}(A)} p^{|x|}(1-p)^{D-|x|} = (1-p)^D \cdot W_{\operatorname{colspace}(A)}(p/(1-p))$. The MacWilliams identity with $t = p/(1-p)$ gives $(1-p)^D W_{\operatorname{colspace}(A)}(t) = 2^{-n} W_N(1-2p)$. For the expectation, $N = \Omega \cdot L$ is the image of a uniform Lagrangian $L$, so $\Pr[x \in N] = 1/(2^n+1)$ for every non-zero $x$ by $\mathrm{Sp}(2n)$-transitivity. Hence $\E_A[W_N(1/2)] = 1 + ((3/2)^D-1)/(2^n+1) \le 1+(9/8)^n$, yielding the claimed bound.
\end{proof}
