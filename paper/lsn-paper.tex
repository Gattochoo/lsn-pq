...
\section{Superseded Approaches to the Marginal-Adaptive Corner}
\label{app:superseded}

Earlier drafts attempted to close the marginal-adaptive corner via Fisher-information and total-variation bounds.  Those arguments targeted $I(x;y)$, the mutual information between the secret $x$ and the noisy output $y$, under the implicit assumption that the reduction matrix $C=BA$ is hidden from the solver.  In the honest threat model, however, $C$ is public; the correct quantity is the \emph{conditional} mutual information $I(x;y|C)$.  Because $I(x;y) \le I(x;y|C) + I(x;C)$ and $I(x;C)$ can be large when $C$ is public, bounds on $I(x;y)$ do not translate into hardness proofs for the public-$C$ setting.  The approaches recorded in this appendix are therefore \textbf{superseded}; they are retained only to prevent redundant exploration.
