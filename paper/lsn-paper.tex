...
Fixed $B$, any rank, $m \ge cn$ & CLOSED & D.1 + Fannes: SD $\geq d-1/(mn)$ \\
Public $B$, $\rho \ge (3/2+\epsilon)n$ & CLOSED & transport/Gram (constructive) \\
Adaptive $B$, conditional-uniform & CLOSED & reachability (\Cref{thm:any-b-uniformity}) \\
...
\Cref{thm:marginal-adaptive} is a \textbf{conditional} barrier: \Cref{lem:m1} is proven, but \Cref{lem:m2} remains open because of the correlated-noise obstacle described above.  The landscape is therefore: fixed-$B$ closed, public-$B$ closed, conditionally-uniform adaptive closed, marginal-adaptive \emph{open}.  The remaining regime $m=\Theta(n)$ with $m<Cn$ is covered by LPQR26 Appendix~D (their Theorem~D.2) for fixed $B$ (any rank, including low-rank).
