...
Recent independent work by Khesin \etal\ \cite{KLP+25}, Poremba--Quek--Shor \cite{PQS26}, and Lu \etal\ \cite{LPQR26} has shown that LSN is at least as hard as constant-rate LPN even for a single logical qubit, and has established strong barriers against reducing symplectic LPN to standard LPN (for linear reductions, proven in the constant-rate sample regime $m=\Theta(n)$; see \Cref{sec:barriers}). These results position LSN as the strongest current candidate for a genuinely new post-quantum hardness family outside the six classical ones (multivariate-quadratic, lattice, code, hash, Ising, tensor).
...
The columns of $A$ may be uniform random, pseudorandom (as in our KEM, \Cref{sec:kem}), or structured. A structurally important special case is \emph{symplectic LPN} (sympLPN)~\cite{KLP+25,LPQR26}, where the columns of $A$ are \emph{isotropic}: they satisfy the deterministic public quadratic relation
\[
S_A \;:=\; A^{\top}\Omega A \;=\; 0,
\]
i.e., every pair of columns is orthogonal under the symplectic form. We denote this variant sympLPN$_{n,m,p}$.

\paragraph{Relationship.} Batch-LSN is \emph{easier} than membership-LSN for the adversary: the query points are known in advance, so each label conveys $\Theta(1)$ bits about $L$ (versus $\Theta(2^{-n})$ in the membership form; see \Cref{sec:info-floors}). Consequently, any hardness result for membership-LSN (SQ lower bounds, brute-force barriers) \emph{carries over} to batch-LSN and to sympLPN: if the harder oracle problem is secure, so is the easier batch variant. The converse reduction is non-trivial and not claimed. Throughout the paper we state which formulation each theorem addresses; unless noted otherwise, ``LSN'' refers to the membership form.
...
\[
I\bigl(L; (a,b)\bigr) \;=\; H_2\bigl(p + 2^{-n}(1-2p)\bigr) - H_2(p) \;=\; \Theta(2^{-n}).
\]
\end{proposition}

\begin{proof}
Given $L$, the label is $b = \mathbf{1}_L(a) \oplus e$ with $e \sim \operatorname{Bernoulli}(p)$, so $H(b \mid a, L) = H_2(p)$ exactly.  Marginalising over $L$, a uniformly random $a$ lies in $L$ with probability $2^{-n}$, hence $\Pr[b=1 \mid a] = p + 2^{-n}(1-2p)$ and $H(b \mid a) = H_2(p + 2^{-n}(1-2p))$.  The difference is $\Theta(2^{-n})$ by the Taylor expansion $H_2(p+\varepsilon) = H_2(p) + \varepsilon\log_2\frac{1-p}{p} + O(\varepsilon^2)$.
\end{proof}
...
A negative answer would be the cleanest quantitative statement of what $\mathrm{LSN}\setminus\mathrm{LPN}$ is; a positive answer would yield a concrete attack improvement (and a step toward a $6.5$th-family demotion).  The SQ lower bounds of \Cref{sec:sq} already rule out enrichment for SQ-implementable selection rules, because any such rule is just a linear combination of SQ queries, and the correlation bounds limit its advantage to $2^{-n}$.
...
\paragraph{Security remark.} Restricting to graph Lagrangians reduces the key-space size by a constant factor $|\mathrm{Sym}(n)| / |\Lagr| \to \prod_{i\ge1}(1+2^{-i})^{-1}\approx1/2.38$, which does not affect the asymptotic security parameter. The SQ lower bounds (Theorems~\ref{thm:main-sq-uncond} and~\ref{thm:main-sq-cond}) and the empirical brute-force barriers (\Cref{sec:decoders}) are proved for membership-LSN; because batch-LSN is easier for the adversary (\Cref{subsec:two-forms}), hardness of the membership form carries over to the batch formulation used here. Correlation structure is preserved under symplectic change of coordinates, so the bounds apply asymptotically unchanged. As a bonus, the secret key is more compact: $M$ requires only $n(n+1)/2$ bits versus $2n^2$ bits for a general basis representation.
...
Linear, $3n/2<\rho<2n$ (near-full) & BLOCKED & Gram rank $\le 2c<n$ (\Cref{thm:transport-nearfull}) \\
Linear, $\rho\le 3n/2$ (mid/low) & OPEN & Entropy regime; LPQR26 App.~D + \Cref{prop:entropy-floor} \\
...
Let $B \in \F_2^{m \times 2n}$ with $\operatorname{rank}(B) = \rho = 2n-c$ where $1 \le c \le n-1$. Let $K = \ker B$ ($\dim K = c$). Then there exists a symmetric matrix $M \in \F_2^{m \times m}$ such that for every isotropic-column matrix $A$,
\[
\operatorname{rank}\bigl((BA)^{\top} M (BA)\bigr) \;\le\; 2c - \operatorname{rank}(\Omega|_K) \;\le\; 2c.
\]
In particular, an adversarial reduction can choose $K$ isotropic (possible for all $c \le n$), in which case the Gram rank of the output is at most $2c$ deterministically. A uniformly random matrix $C \in \F_2^{m \times n}$ satisfies $\operatorname{rank}(C^{\top} M C) \le 2c$ with probability at most $2^{-\Omega(n^2)}$ whenever $2c \le n - \Theta(1)$.
\end{theorem}

\begin{proof}
Write $B = P^{\top}$ with $P \in \F_2^{\rho \times 2n}$ of rank $\rho$. The set of symmetric matrices $N$ of the form $P^{\top}QP$ ($Q$ symmetric) is exactly the set of symmetric bilinear forms vanishing on $K$ from both sides. Fix a basis adapted to $K \oplus V$ with $\dim V = 2n-c$. In this basis the standard symplectic form $\Omega$ becomes a block matrix with blocks $\Omega_{KK}, \Omega_{KV}, \Omega_{VK}, \Omega_{VV}$. Adding a form $N$ that kills $K$ on both sides changes only the $VV$ block; the blocks $\Omega_{KK}, \Omega_{KV}, \Omega_{VK}$ remain fixed. We therefore obtain a \emph{minimal-rank completion} problem for the matrix
\[
...
\]
The minimum is achieved constructively by $E_{VV} = \Omega_{VK} \cdot G \cdot \Omega_{KV}$ with $G$ any generalized inverse of $\Omega_{KK}$ (over $\F_2$, obtained from a rank factorization). Setting $M$ to the matrix representing this optimal form in the original coordinates yields the first claim.

For the probability bound, when $2c \le n-\Theta(1)$ the rank bound $2c$ is at most $n-\Theta(1)$. The uniform Gram $C^{\top}MC$ is an $n \times n$ symmetric matrix over $\F_2$; its rank is at least $n-O(1)$ with probability $1-2^{-\Omega(n^2)}$ (standard random symmetric matrix rank over $\F_2$). Hence the rank-$\le 2c$ event has exponentially small probability under the uniform distribution.
\end{proof}
...
\item $3n/2 < \rho < 2n$ (near-full): \textbf{BLOCKED} --- output Gram rank $\le 2c < n$ (\Cref{thm:transport-nearfull}), while the uniform Gram has rank $\approx n$.
\item $\rho \le 3n/2$ (mid/low rank): \textbf{OPEN} --- the rank test dies ($2c \ge n$), and the barrier passes to the entropy-deficiency regime of Lu \etal\ \cite{LPQR26} (Appendix~D, Theorem~D.1) and the entropy argument of \Cref{prop:entropy-floor}. A quantitative bridge theorem that covers \emph{all} $B$-distributions simultaneously would close this strip entirely.
\end{itemize}
This stratification applies to \emph{fixed/public} $B$ (per-realization). If the reduction draws $B$ from a high-entropy distribution conditioned on $BA$ being uniform, the matrix is randomized---but then the label noise $Be$ is killed by piling-up at $p=1/4$ (row weight $w$ gives bias $2^{-w}$). The two regimes are endpoint observations; the quantitative trade-off over all $B$-distributions is an open problem (see \Cref{sec:open}).
...
\begin{enumerate}
\item \textbf{Statistical dimension under $S_A=0$ for sympLPN \cite{LPQR26}.} The full SQ proof is blocked by the deterministic quadratic symplectic relation $S_A=0$ (the $\Omega$-Gram of columns) that characterises the sympLPN variant (\Cref{def:lsn-batch}). Prove that conditioning on $S_A=0$ does not reduce the statistical dimension below $2^{\Omega(n)}$.
