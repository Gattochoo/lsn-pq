...
a $\delta$-enrichment would scale the per-query advantage by $(1+\delta)$; for selection rules implementable by linear SQ this exceeds the per-query ceiling of \Cref{thm:linear-sq}, while general (non-linear, multi-sample) selection rules remain open.
