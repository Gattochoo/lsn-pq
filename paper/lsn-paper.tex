...
\Cref{thm:marginal-adaptive} is a \textbf{conditional} barrier: \Cref{lem:m1} is proven, but \Cref{lem:m2} remains open because of the correlated-noise obstacle described above.  The honest map is therefore: fixed-B \textbf{DEAD}, public-B \textbf{DEAD}, conditional-uniform adaptive \textbf{DEAD}, marginal-adaptive \textbf{OPEN}.  The remaining regime $m=\Theta(n)$ with $m<Cn$ is covered by LPQR26 Appendix~D (their Theorem~D.2) for fixed $B$ (any rank, including low-rank).
