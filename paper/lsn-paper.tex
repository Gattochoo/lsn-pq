...
\Cref{lem:enrichment-sq} shows that any selection rule achieving $\delta = \Omega(1)$ yields an SQ distinguisher with advantage $\Omega(2^{-n})$, which is exactly the threshold ruled out by \Cref{thm:linear-sq} for linear SQ algorithms. Non-linear selection rules are not covered by this lemma; their analysis remains open.
