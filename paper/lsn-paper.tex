...
Recent independent work by Khesin \etal\ \cite{KLP+25}, Poremba--Quek--Shor \cite{PQS26}, and Lu \etal\ \cite{LPQR26} has shown that LSN is at least as hard as constant-rate LPN even for a single logical qubit, and has established strong barriers against reducing symplectic LPN to standard LPN (for linear reductions, tightly in the constant-rate sample regime $m=\Theta(n)$; see \Cref{sec:barriers}). These results position LSN as the strongest current candidate for a genuinely new post-quantum hardness family outside the six classical ones (multivariate-quadratic, lattice, code, hash, Ising, tensor).
...
Linear sample-red., $m=\omega(n)$ & OPEN & LPQR26 bound insufficient (their caveat) \\
