...
Post-quantum cryptography currently rests on a handful of established hardness families: lattices (LWE/SIS), error-correcting codes (LPN/syndrome decoding), multivariate quadratic equations (MQ), hash functions, and isogenies, with tensor and group-action problems emerging as further candidates. A genuinely new family --- structurally distinct and not reducible to the existing ones --- has been sought since the inception of the field. LSN is a leading candidate for such a family.
