...
\begin{enumerate}
    ...
    \begin{itemize}
        \item \textbf{LSN-SNARK:} A preprocessing SNARK for LSN membership with $O(n^2)$ R1CS constraints via the Siegel-chart (graph-Lagrangian) representation. Verified counts: $64$ for $n=8$, $1{,}681$ for $n=41$, $4{,}225$ for $n=65$.
...
Code-based cryptography dates to McEliece \cite{McE78} and Prange \cite{Pra62}. Modern standards include Classic McEliece \cite{ABD+21}, HQC \cite{AAC+22}, and BIKE. Polar codes, introduced by Arikan \cite{Ari09}, are capacity-achieving with explicit construction; list decoding was introduced by Tal and Vardy \cite{TV15} and refined by Fazeli and Vardy \cite{FV19}, and finite-length analysis was developed by Mori--Tanaka \cite{MT09}, Trifonov \cite{Tri12}, and Hassani \etal\ \cite{HVU14}. We use Bhattacharyya bounds for conservative parameter selection \cite{Sei15}.

\paragraph{Post-quantum KEMs.}
The NIST PQC standardization process \cite{NIS22} selected Kyber (lattice) \cite{SAB+22}, Classic McEliece, and HQC \cite{AAC+22} as KEM candidates. LSN-KEM offers competitive sizes with a fundamentally different assumption.
...
\hline
80-bit  & 41  & 80.4  & $\sim 2^{861}$  & 1.78 KB \\
128-bit & 65  & 128.4 & $\sim 2^{2145}$ & 2.78 KB \\
...
\paragraph{Implementation note.} A small-scale Python prototype of the SCL decoder exhibited a bug (BLER = 1.0 for $N=128,256$), contradicting the conservative Bhattacharyya bound and indicating an implementation issue rather than a parameter problem. Subsequent validation with the verified \texttt{komm} SC decoder \cite{komm} revealed a second bug in the Bhattacharyya parameter indexing: the original code concatenated all bad channels before all good channels, whereas \texttt{komm}'s natural-order polar code requires interleaving ($z_{2i}=2z_i-z_i^2$, $z_{2i+1}=z_i^2$). After correcting the frozen-set construction, all tested configurations yielded BLER $=0$: at $p'=0.0706$ for $N=128,256,512$ with $K=N/8$, and at $p'=0.0343$ for all tested lengths. The previous $N=256$ anomaly (BLER $=0.115$) was entirely due to the indexing mismatch, not incomplete polarization.

A production-grade Rust validation decoder (SCL with list size $L=8$, min-sum path metric) was subsequently exercised at the design length $N=2048$, $K=256$ for $2000$ independent trials at both noise points ($p'=0.0706$ and $p'=0.0343$). No block errors were observed; under the naive binomial model the one-sided $95\%$ upper confidence bound for the BLER is $1.5\times10^{-3}$. High-noise negative controls ($p'\in\{0.3,0.4,0.5\}$) on the same harness exhibited near-total or total failure (BLER $0.965$--$1.000$), confirming that noise injection and block comparison are active. These results strengthen the empirical support for the $N=2048$ parameters but do not replace the conservative Ar\i{}kan--Bhattacharyya bound ($2^{-80}$ at $r=7$, $2^{-148}$ at $r=11$), which remains the design basis; a direct Monte-Carlo validation of $2^{-80}$ would require infeasibly many trials.
...
Fixed $B$, any rank, $m \ge cn$ & DEAD & D.1 + Fannes: SD $\geq d-1/(mn)$ \\
Public $B$, $\rho \ge (3/2+\epsilon)n$ & DEAD & transport/Gram (constructive) \\
Adaptive $B$, conditional-uniform & DEAD & reachability (\Cref{thm:any-b-uniformity}) \\
Adaptive $B$, marginal-uniform, deterministic & \textbf{DEAD} & support counting: $|\operatorname{supp}(BA)|\le|\Lagr(2n,\F_2)|$ (\Cref{thm:deterministic-marginal-adaptive}) \\
...
\item $\rho \le (3/2-\epsilon)n$ (mid/low rank): \textbf{DEAD} for fixed $B$ at $m \ge cn$ --- the rank test dies, but the output matrix $BA$ is statistically far from uniform. By LPQR26's Theorem~D.1, $H(BA) \le (1-d)mn$ for a constant $d > 0$; the Fannes--Csisz\'{a}r continuity inequality gives $\operatorname{SD}(BA, \operatorname{Uniform}) \ge d - 1/(mn)$. At $n=41$, $m=82$ ($m=2n$), the frame entropy $H(A) \le (3/2)n^2 + n/2$ gives $d \ge 1/4 - 1/(4n) \approx 0.244$, so this distance is at least $0.24$. Since any LPN solver expects a uniform matrix, the reduction fails regardless of rank. The rank-stratification theorems above retain value as \emph{constructive} distinguishers (explicitly exhibiting the defect), whereas the Fannes argument is non-constructive but covers all ranks uniformly.
\end{itemize}
...
The deterministic half of the marginal-adaptive cell is closed unconditionally by a support-counting argument.

\begin{theorem}[Deterministic marginal-adaptive barrier]
\label{thm:deterministic-marginal-adaptive}
Let $A\sim\operatorname{Uniform}(\Lagr(2n,\F_2))$ and let $B=g(A)\in\F_2^{m\times 2n}$ be any deterministic function of $A$.  Set $C=BA$.  Then for every noise rate $p'$,
\[
...
\]
In particular, for $m\ge n+3$ the right-hand side is $1-o(1)$.
\end{theorem}

\begin{proof}
Since $C$ is a deterministic function of $A$, its support has size at most $|\Lagr(2n,\F_2)|$, whereas the LPN public matrix is uniform on a set of size $2^{mn}$.  The statistical distance between a distribution supported on $K$ elements and the uniform distribution on $N$ elements is at least $1-K/N$.  The pair $(C,y)$ determines $C$, so by data processing the same lower bound holds for $\operatorname{SD}((C,y),\mathrm{LPN}_{p'})$.
\end{proof}

For $n=2$ the bound is $1-15/4^m$; Experiments~185 and~186 verify that it is tight for $m=3$ ($49/64$) and $m=4$ ($241/256$).
...
\textbf{Depends on \Cref{lem:m2}.}  Let $A\sim\operatorname{Uniform}(\Lagr(2n,\F_2))$ and let $B=g(A,R)\in\F_2^{m\times 2n}$ be any randomized adaptive matrix (i.e., $B$ may use fresh private randomness $R$).  \Cref{lem:m1} shows that if $\operatorname{SD}(BA,\text{Uniform})\le\delta$ and $m\ge Cn$ ($C>16$), at least $m-o(m)$ rows have weight $>0.19n$, hence effective noise bias $\le 2^{-0.19n}$.  If \Cref{lem:m2} holds, this yields $\operatorname{SD}((C,y),\mathrm{LPN}_{p'})=1-o(1)$ for every $p'$ with $1-2p'\ge 1/\operatorname{poly}(n)$, and no randomized marginal-adaptive linear reduction maps symplectic LPN to a usable standard LPN instance when $m\ge Cn$.
\end{theorem}

\Cref{thm:marginal-adaptive} is a \textbf{conditional} barrier for the randomized case: \Cref{lem:m1} is proven, but \Cref{lem:m2} remains open because of the correlated-noise obstacle described above.  The deterministic marginal-adaptive case is already closed unconditionally by \Cref{thm:deterministic-marginal-adaptive}.  The honest map is therefore: fixed-$B$ \textbf{DEAD}, public-$B$ \textbf{DEAD}, conditionally-uniform adaptive \textbf{DEAD}, deterministic marginal-adaptive \textbf{DEAD}, randomized marginal-adaptive \textbf{OPEN}.  The remaining regime $m=\Theta(n)$ with $m<Cn$ is covered by LPQR26 Appendix~D (their Theorem~D.2) for fixed $B$ (any rank, including low-rank).
...
\begin{enumerate}
\item \textbf{Empirical gap at $N=2048$.} The concatenated code is designed via the Bhattacharyya bound, which guarantees $P_e\leq 2^{-80}$ for the target block length $N=2048$. We have validated a Rust SCL decoder ($L=8$, min-sum) empirically for $N\in\{128,256,512\}$ (all BLER $=0$ in 200 trials) and directly at $N=2048$ ($2000$ trials at $p'=0.0706$ and $p'=0.0343$, $0$ block errors, one-sided $95\%$ BLER upper bound $1.5\times10^{-3}$), with high-noise negative controls confirming the harness. The design-point claim $P_e\leq 2^{-80}$ itself still rests on the Bhattacharyya bound rather than direct measurement, since certifying $2^{-80}$ by Monte Carlo is infeasible.
...
This appendix summarizes the SQ/feature-map barriers; for the full linear-reduction landscape (transport, entropy-continuity, reachability, deterministic marginal-adaptive, and the open randomized marginal-adaptive cell) see \Cref{tab:barriers} and \Cref{sec:barriers}.
