...
A $\delta$-enrichment would scale the per-query advantage by $(1+\delta)$; for selection rules implementable by linear SQ this exceeds the per-query ceiling of \Cref{thm:linear-sq}, while general (non-linear, multi-sample) selection rules remain open.  Empirical probes at $n=5$ (full graph-Lagrangian prior, $32{,}768$ secrets) and $n=6$ (random subset, $100{,}000$ secrets) confirm the predicted scaling $\delta \approx c\,m\,2^{-n}$ with $c=\Theta(1)$ (direct measurement gives $c\approx 0.01\text{--}0.06$ in the measured regime, while a chi-squared-unit analysis suggests $c\approx 0.7\text{--}0.9$; the exact constant depends on the prior and sample-size regime, but the exponential scaling in $n$ is the operative fact).  At $n=65$ and $m=22{,}528$ this gives $\delta \le 2^{-50}$, far below any cryptographically relevant threshold.
