...
(1) \textbf{표준모형 격차:} LSN 하드니스는 원시 가정이다 --- LWE급 worst-case 환원이 없다. (2) \textbf{SQ는 증거이지 증명이 아니다:} SQ 하계는 LPN·LWE에도 있으므로 family 구별자가 아니다. (3) \textbf{폴라 검증:} $N\le512$ 및 $N=2048$(2000회, 블록오류 0, 95\% 상계 $1.5\times10^{-3}$)을 Rust로 실증했으나 설계점 $2^{-80}$ 자체는 Bhattacharyya 한계에 의존한다. (4) \textbf{느슨한 다중사용자 환원:} 정확한 비밀 재무작위화는 가능하지만 표본 신선도가 막혀 있어($\mathbf{1}_L$ 비선형성) 하이브리드가 $N$배를 잃는다. 실무 여유는 충분하다($q \gtrsim 2^{114}$). (5) \textbf{실용 최적화 유보:} $p=1/4$ 고정, 다수결 디코더(상수시간) 등은 엔지니어링 자유도다.
