#!/usr/bin/env python3
"""179: Scaling test — noise-side I(e';C) at n=3, m=7.

Uses exact enumeration of all 22680 isotropic 3x6 matrices A and
incremental entropy updates to make SA feasible.

Run: python3 experiments/179-KIMI-noise-side-scaling-n3.py
Status: active.
"""
import random
import math
...
def symplectic_form(v, w, n):
    low = (1 << n) - 1
    return (
        bin((v & low) & (w >> n)).count("1")
        ^ bin((v >> n) & (w & low)).count("1")
    ) & 1
...
class State:
    """Incremental I(e';C) and marginal cost."""
    ...
    def _update_H(self):
        ...
        for C_tuple, p_C in self.marginal_C.items():
            ...
            self.H_e_given_C += p_C * H_cond

    def _H_cond(self, C_tuple):
        p_C = self.marginal_C.get(C_tuple, 0.0)
        if p_C <= 0:
            return 0.0
        H = 0.0
        for e, p in self.joint_by_C.get(C_tuple, {}).items():
    ...
    def _update_A(self, A, old_B, new_B):
        ...
        old_contrib_oldC = self.marginal_C.get(old_C_tuple, 0.0) * self._H_cond(
            old_C_tuple
        )
        old_contrib_newC = self.marginal_C.get(new_C_tuple, 0.0) * self._H_cond(
            new_C_tuple
        )
        ...
        if self.marginal_C[old_C_tuple] == 0:
            del self.marginal_C[old_C_tuple]
            del self.joint_by_C[old_C_tuple]
        ...
        new_contrib_oldC = self.marginal_C.get(old_C_tuple, 0.0) * self._H_cond(
            old_C_tuple
        )
        new_contrib_newC = self.marginal_C.get(new_C_tuple, 0.0) * self._H_cond(
            new_C_tuple
        )
        self.H_e_given_C += (
            new_contrib_oldC
    ...
    def compute_I(self):
        return self.H_e - self.H_e_given_C
    ...
    for it in range(max_iters):
        ...
        if new_cost <= current_cost:
            ...
            if new_cost < best_cost:
                best_cost = new_cost
                if it % 500_000 == 0:
                    print(f"  ...it {it}, cost={best_cost}, I={state.compute_I():.5f}")
        else:
        ...
        if dE <= 0 or rng.random() < math.exp(-dE / T):
            ...
            if I < best_I:
                ...
                if improvement > 0.001 or it % 50_000 == 0:
                    print(
                        f"  *** NEW BEST I={best_I:.6f}, marg={best_marg} at it {it}"
                    )
        else:
            state._update_A(A, new_B, old_B)
...
def main():
    ...
    for lam in [0.05, 0.1]:
        ...
        print(
            ...
            f"I={state.compute_I():.6f}, marg={state.marginal_cost()}"
        )

        seed = 2000 + int(lam * 1000)
        ...
        results.append(
            {"lambda": lam, "best_I": best_I, "best_marg": best_marg, "best_E": best_E}
        )
    ...
    out_file = out_dir / "179-scaling-n3-results.json"
